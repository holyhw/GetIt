...
export function LocationSearchModal({ visible, token, onSelect, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex md:items-center md:justify-center md:bg-black/50">
      {/* 배경 클릭 닫기 (데스크탑) */}
      <div className="absolute inset-0 hidden md:block" onClick={onClose} />

      {/* 모달 본체 */}
      <div className="relative w-full h-full bg-white flex flex-col md:w-[480px] md:h-[560px] md:rounded-2xl md:shadow-xl">
        {/* 헤더 */}
        <div className="flex items-center px-5 pt-8 pb-3 gap-3 md:pt-5">
          <button onClick={onClose} className="cursor-pointer bg-transparent border-none p-1 shrink-0 md:hidden">
          ...
          <button onClick={onClose} className="hidden md:block cursor-pointer bg-transparent border-none p-1 shrink-0 text-app-gray hover:text-black transition-colors text-xl leading-none">×</button>
        </div>
