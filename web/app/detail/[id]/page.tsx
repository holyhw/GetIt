...
export default function DetailPage() {
  ...
  return (
    <div className="relative min-h-dvh bg-white">
      {/* 히어로 이미지 */}
      <div className="relative w-full h-[317px] overflow-hidden bg-white">
        {item.imageUrl ? (
          <Image
            src={item.imageUrl}
            alt=""
        ...
        <div className="absolute top-8 left-6 z-10">
          <DarkBtn onClick={() => fromRegistration ? router.replace("/") : router.back()}>
            <BackIcon />
          </DarkBtn>
        </div>
        <div className="absolute top-8 right-[70px] z-10">
          <DarkBtn onClick={handleShare}>
            <ShareIcon />
          </DarkBtn>
        </div>
        <div className="absolute top-8 right-6 z-10">
          <DarkBtn onClick={() => setShowMore(true)}>
            <MoreIcon />
          </DarkBtn>
        </div>
      </div>
      ...
      {/* 컨텐츠 */}
      <div className="mx-1 rounded-t-[20px] bg-app-bg px-6 pt-6 pb-32 shadow-[0_-2px_4px_rgba(0,0,0,0.05)]" style={{ marginTop: -1 }}>
        {/* 카테고리 태그 */}
        <div className="inline-flex items-center bg-[#E5E5E5] rounded-full px-3 h-[27px] mb-6">
      ...
      {/* 하단 CTA */}
      <div className="fixed bottom-0 left-0 right-0 bg-app-bg px-7 pt-3 pb-6 z-50">
        <button
