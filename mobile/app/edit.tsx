...
export default function EditScreen() {
  ...
  useEffect(() => {
    ...
    api.get<RegistrationDetail>(`/api/registration/${id}`, token ?? "")
      ...
      .catch(() => Alert.alert("오류", "게시물을 불러올 수 없습니다."))
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/registration/${id}?${params}`, {
        method: "PATCH",
  ...
  const displayImage = photo ?? existingImageUrl;
      ...
      <View style={{ flex: 1, backgroundColor: "#F5F7FA" }}>
        ...
        <View style={{ backgroundColor: "#F5F7FA" }}>
          ...
          <View style={{ height: 51, flexDirection: "row", alignItems: "center", paddingHorizontal: 24 }}>
            <TouchableOpacity onPress={() => router.back()} style={{ marginRight: 8 }} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}>
              <BackIcon width={11} height={19} />
            </TouchableOpacity>
            <View style={{ flex: 1, alignItems: "center" }}>
              <Text style={{ fontSize: 20, fontWeight: "500", color: "#000", letterSpacing: -0.32 }}>물건 수정하기</Text>
            </View>
            <View style={{ width: 19 }} />
          </View>
        </View>
          ...
          <View style={{
            ...
            <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 10 }}>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                ...
                <Text style={{ fontSize: 10, color: "#919191", marginLeft: 3 }}>{displayImage ? "(1/1)" : "(0/1)"}</Text>
                <Text style={{ fontSize: 10, fontWeight: "700", color: "#919191", marginLeft: 2 }}>{" (선택)"}</Text>
              </View>
              <View style={{ flex: 1 }} />
            ...
            {!displayImage ? (
              ...
              <View style={{ flexDirection: "row", gap: 12 }}>
                ...
                <View>
                  <Image
                    source={photo ? { uri: photo } : { uri: existingImageUrl! }}
                    style={{ width: 131, height: 131, borderRadius: 15 }}
                    resizeMode="cover"
                  />
                  <TouchableOpacity onPress={removePhoto} style={{ position: "absolute", top: -6, right: -6 }}>
        ...
        <View style={{ backgroundColor: "#F5F7FA", paddingHorizontal: 28, paddingTop: 12, paddingBottom: 22 }}>
          ...
          >
            {loading ? <ActivityIndicator color="#fff" /> : <Text style={{ color: "#fff", fontSize: 14, fontWeight: "600", letterSpacing: -0.32 }}>수정 완료</Text>}
          </TouchableOpacity>
        </View>
